...
public class AlphaBetaRealMoveOrdering implements MoveStrategy {
	...
	public AlphaBetaRealMoveOrdering(final int depth) {
	...
	public Move execute(final Board board) {
		...
		while(timer < 5000){
			...
			for (final Move move : moveSort(board, depth)){/*this.moveSorter.sort(board.currentPlayer().getLegalMoves())*/ 
		...
		if(!(previousBestMove == null)){
			...
			System.out.println("CHOSEN MOVE IS " + errorMove);
			this.depth = currentDepth-2;
			return errorMove;
		}
	}
